...
public class AppTest {
    ...
    void testGetAllUsers() throws Exception {
        var response = mockMvc.perform(get("/people")).andReturn().getResponse();
        assertThat(response.getStatus()).isEqualTo(200);
    }

    @Test
    void testGetUserById() throws Exception {
        var response = mockMvc.perform(get("/people/{id}", 1))
    ...
    void testDeleteUserById() throws Exception {
        var responseDelete = mockMvc.perform(delete("/people/{id}", 1))
                .andReturn().getResponse();
        assertThat(responseDelete.getStatus()).isEqualTo(200);

        var responseGetAll = mockMvc.perform(get("/people")).andReturn().getResponse();
        assertThat(responseGetAll.getContentAsString()).doesNotContain("Smith");
    }
    // END
